...
## 1. podman build --platform linux/x86-64 -t am-ubuntu -f am-ubuntu.dockerfile
## 2. podman run -it --platform linux/x86-64 --name am-ubuntu-test --device /dev/fuse --cap-add SYS_ADMIN --security-opt unmask=ALL --tmpfs /opt --tmpfs /root/.local/share/applications am-ubuntu:latest
...
RUN echo "echo \"AM-Ubuntu testing container started\" && . test-common.sh && _update" >> ~/.bashrc
