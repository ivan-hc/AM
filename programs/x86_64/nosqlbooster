...
version=$(echo $(wget -q https://aur.archlinux.org/packages/nosqlbooster-mongodb -O - | grep -Eo "(http|https)://[a-zA-Z0-9./?=_%:-]*" | grep -i ".appimage" | grep -w -v ".asc" | grep -w -v ".zsync" | grep -w -v "archlinux.org" | head -1))
wget $version
