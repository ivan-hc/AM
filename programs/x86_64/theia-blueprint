...
version=$(curl -Ls https://www.eclipse.org/downloads/packages/ | tr '"' '\n' | grep -i "/release/" | head -1 | sed 's:.*/::')
wget "https://download.eclipse.org/theia/latest/linux/TheiaBlueprint.AppImage" || exit 1
...
if [ "$version" != "$version0" ] || [ -e /opt/"$APP"/*.zsync ]; then
	...
	[ -e ../*.zsync ] && wget "$version.zsync" 2>/dev/null || { wget "https://download.eclipse.org/theia/latest/linux/TheiaBlueprint.AppImage" || exit 1; }
	# Use tar fx ./*tar* here for example in this line in case a compressed file is downloaded.
	cd ..
