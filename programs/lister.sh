...
for arch in $DIRS; do
	...
	if [ "$arch" = x86_64 ]; then
		...
		for m in $METAPACKAGES; do
			...
			metapkg_page=$(curl -Ls --retry 5 --retry-max-time 120 "https://raw.githubusercontent.com/Portable-Linux-Apps/Portable-Linux-Apps.github.io/refs/heads/main-backup/apps/$m.md" 2>/dev/null)
